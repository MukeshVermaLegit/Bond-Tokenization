...
contract BondToken is ERC20 {
    ...
    /// Calculate accrued interest for an address
    function pendingInterest(address user) public view returns (uint256) {
        uint256 holderBalance = balanceOf(user); // no. of bonds held
        ...
        return monthsElapsed * monthlyInterest - claimedInterest[user]; // units in USDC
    }
